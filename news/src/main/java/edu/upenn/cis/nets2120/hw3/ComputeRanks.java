...
public class ComputeRanks {
	 ...
	 */
	JavaPairRDD<String, String> getArticleToCategory(String filepath) {
		...
		// System.out.println(graph.take(20));
		return graph;
	}
	 ...
	 */
	JavaPairRDD<String, String> getCategoryToArticle(String filepath) {
	...
	JavaPairRDD<String, String> getFriends() {
		...
		// ScanRequest scanRequest = new ScanRequest()
		// 	.withTableName("users")
		// 	.withFilterExpression("user_relation = friend");

			// .withKeyConditionExpression("begins_with(user_relation, :fh)")
			// .withValueMap(new ValueMap()
			// 			.withString(":fh", "FRIEND#")
			// );
	...
	JavaPairRDD<String, String> getArticleToUser() {
		...
		HashMap<String, AttributeValue> map = new HashMap<>();
		map.put(":l", new AttributeValue("LIKE"));
		ScanRequest scanRequest = new ScanRequest()
			.withTableName("users")
			.withFilterExpression("begins_with(user_relation, :l)") // LIKE#
			.withExpressionAttributeValues(map); 
	...
	public void run(String filepath) throws IOException, InterruptedException {
		...
		JavaPairRDD<String, String> aToC = getArticleToCategory(filepath);
		JavaPairRDD<String, String> cToA = getCategoryToArticle(filepath);
		...
		*/
		...
		for (int i = 0; i < maxIters; i++) {
			...
			// sum up all of them with same label & drop labels less than certain value
			double min = 0.005;
			// ((p, u), val)
			...
			temp = temp.filter(item -> item._2 > min);
			...
			// remove lable nodes on users that match the user
		...
		this.writeToDb(rankRDD);

		logger.info("*** Finished social network ranking! ***");
	}

	public void writeToDb(JavaPairRDD<String, Tuple2<String, Double>> rdd) {
		...
		// HashMap<String, AttributeValue> map = new HashMap<>();
		// map.put(":u", new AttributeValue("article"));
		// ScanRequest scanRequest = new ScanRequest()
		// 	.withTableName("users")
		// 	.withFilterExpression("begins_with(user_relation, :u)") // LIKE#
		// 	.withExpressionAttributeValues(map); 

		// ScanResult result = db2.scan(scanRequest);
		// for (Map<String, AttributeValue> item : result.getItems()){
		// 	Map<String, AttributeValue> x = new HashMap<>();
		// 	x.put("id", item.get("id"));
		// 	x.put("user_relation", item.get("user_relation"));
			
		// 	DeleteItemRequest req = new DeleteItemRequest().withTableName("users")
		// 	.withKey(x);
